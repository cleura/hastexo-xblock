...
from stackamole.stackamole import StackamoleXBlock
...
class TestStackamoleXBlockHTML(TestCase):
    """
    ...
    def test_static(self):
        ...
        for static_file in static_files:
            etree.parse(
                os.path.join('stackamole', 'static', 'html', static_file),
                etree.HTMLParser(recover=False))


class TestStackamoleXBlockParsing(XmlTest, TestCase):
...
class TestStackamoleXBlock(TestCase):
    """
    Basic unit tests for the Stackamole XBlock.
    ...
    def setUp(self):
        ...
        self.block = StackamoleXBlock(runtime,
                                      field_data,
                                      scope_ids=scope_ids)
    ...
    def test_xblock_export_to_separate_file(self):
        ...
        export_content = export_fs.readtext('stackamole/fake_lab.xml')
    ...
    def test_full_round_import_export(self):
        ...
        # create the stackamole node
        stackamole_node = etree.Element(block_type)
        stackamole_node.set('filename', block_id)
        stackamole_node.set('stack_user_name', 'training')

        # run the import
        block = self.block.parse_xml(
            stackamole_node, self.block.runtime, scope_ids)

        # export the xblock to fake_lab.xml
        # setup
        block.url_name = 'fake_lab'
        block.category = 'stackamole'
        export_fs = OSFS('fake/course')
    ...
    def test_export_import_export(self):
        ...
        self.assertTrue(export_fs.exists('stackamole/fake_lab_1.xml'))
        export_content_1 = export_fs.readtext('stackamole/fake_lab_1.xml')
        ...
        # assert that the exported file exists and read the content
        self.assertTrue(export_fs.exists('stackamole/fake_lab_2.xml'))
        export_content_2 = export_fs.readtext('stackamole/fake_lab_2.xml')
        ...
        # clean up
        export_fs.remove('stackamole/fake_lab_1.xml')
        export_fs.remove('stackamole/fake_lab_2.xml')
        export_fs.removedir('stackamole')
