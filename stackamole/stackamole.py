...
class StackamoleXBlock(XBlock,
    ...
    def validate(self):
        ...
        if block_type == "hastexo":
            validation.add(ValidationMessage(
                ...
                "Using the 'hastexo' module is deprecated. Please "
                "update this course to use the 'stackamole' module instead."
            ))
        return validation
    ...
    def parse_xml(cls, node, runtime, keys):
        ...
        if 'filename' in node.attrib:
            ...
            for category in ('stackamole', 'hastexo'):
                pointer_path = "{category}/{url_path}".format(
                    category=category,
                    url_path=location.block_id.replace(':', '/')
                )
                base = os.path.dirname(pointer_path)
                filepath = u"{base}/{name}.xml".format(
                    base=base, name=filename)

                if runtime.resources_fs.exists(filepath):
                    break
            else:
                raise FileNotFoundError(
                    f"Could not find '{filename}.xml' under 'stackamole' "
                    "or 'hastexo' categories."
                )
    ...
    def _set_module_name(self):
        ...
        if block_type == "hastexo":
            ...
            try:
                ...
                advanced_modules = list(course.advanced_modules or [])
                # remove 'hastexo'
                advanced_modules = [
                    m for m in advanced_modules if m != 'hastexo']
                # add 'stackamole' if not present
                if self.CATEGORY not in advanced_modules:
                    advanced_modules.append(self.CATEGORY)
                course.advanced_modules = advanced_modules
                course.save()
            except Exception:
                logger.warning(
                    "Unable to update advanced modules list in course policy.")
